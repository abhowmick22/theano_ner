...
class model_birnn_unstructured(object):
    def __init__(self, nh, nc, ne, np, nch, de, dp, dch, cs):
        ...
        self.names  = ['embeddings', 'pos_embeddings', 'chunk_embeddings', 'Wx', 'Wpos', 'Wchunk', 'Wh_forward', 'Wh_backward', 'Wc_forward', 'Wc_backward',
...
class model_birnn_structured(object):
    def __init__(self, nh, nc, ne, np, nch, de, dp, dch, cs, decode='greedy'):
        ...
        # bundle
        self.params = [ self.emb, self.pos_emb, self.chunk_emb, self.Wx, self.Wpos, self.Wchunk,
                        self.Wh_forward, self.Wh_backward, self.Wc_forward, self.Wc_backward,
                        self.Wout, self.bs_forward, self.bs_backward, self.bc, self.c, self.s0_forward, self.s0_backward]
        self.names  = ['embeddings','pos_embeddings', 'chunk_embeddings', 'Wx', 'Wpos', 'Wchunk', 'Wh_forward', 'Wh_backward', 'Wc_forward', 'Wc_backward',
                       'Wout', 'bs_forward', 'bs_backward', 'bc', 'c', 's0_forward', 's0_backward' ]
        ...
        def recurrence(x_forward_t, x_backward_t, p_forward_t, p_backward_t, ch_forward_t, ch_backward_t, s_forward_tm1, s_backward_tm1, y_tm1):
            s_forward_t = T.nnet.sigmoid(T.dot(x_forward_t, self.Wx) + T.dot(p_forward_t, self.Wpos) + T.dot(ch_forward_t, self.Wchunk)
                                        + T.dot(s_forward_tm1, self.Wh_forward)
                                        + self.bs_forward)
            s_backward_t = T.nnet.sigmoid(T.dot(x_backward_t, self.Wx) + T.dot(p_backward_t, self.Wpos) + T.dot(ch_backward_t, self.Wchunk)
